...
| 🔧 **`$pr-fix` 自动修复** | 扫描自己 PR 的 RC comments → 修复 → 2/4-round 自我 review → push → 请求复审 |
...
### `$pr-fix` — 自动修复自己 PR 的 RC（配套 Skill）

`$pr-daemon-loop` 负责 **review 别人的 PR**；`$pr-fix` 负责 **修复自己 PR 的 review comments**。两者配合，构成完整的「提 PR → 被 review → 自动修复 → 再 review」闭环。

```
$pr-fix                          # 自动扫全部 3 个组织，找 RC 或有 comment 的 PR
$pr-fix MushroomDAO/CometENS     # 只扫一个仓库
...
使用 `$pr-fix` 的 token 开销，对比「直接 cd 进业务仓库，手动启动 Claude Code 修改」：

| 模型/步骤 | 直接进仓库 | `$pr-fix` 2-round | `$pr-fix` 4-round |
|-----------|-----------|-------------------|-------------------|
| DeepSeek R1（PR-Daemon 侧） | 0 | ~13k tok | ~13k tok |
| Sonnet R2 挑战 | 0 | 0 | ~10k tok |
| Codex PK | 0 | 0 | ~13k tok |
| Opus 最终拍板 | 0 | 0 | ~21k tok |
...
| `pr-fix` | `$pr-fix` 或 `$pr-fix OWNER/REPO#N` | 自动修复 jhfnetboy 自己 PR 的 RC/review comments |
...
│       ├── pr-fix/SKILL.md           ← 自动修复自己 PR 的 RC
│       └── pr-daemon-status/SKILL.md ← 进度+token 看板
├── scripts/
│   ├── poll_prs.py             ← 全组织 PR 发现 + --sync 入库（GraphQL 批量）
│   ├── poll_fix_queue.py       ← 发现 jhfnetboy 自己 PR 里的 RC/review comments（$pr-fix 入口）
