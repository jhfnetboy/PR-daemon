...
| 类型 | 来源 | 动作 |
|------|------|------|
| **jhfnetboy PRs** | `jhfnetboy` 本人提交 | 扫 RC/comment → 修代码 → loop 直到 APPROVE |
| **Bot PRs** | `dependabot[bot]`、`renovate[bot]` | inline review → 无 blocking → `gh pr merge --squash`；有 RC → Tier C 报告 |

```
$pr-fix                          # 扫全部 3 个组织，两类 PR 都处理
$pr-fix MushroomDAO/CometENS     # 只扫一个仓库
$pr-fix MushroomDAO/CometENS#4   # 只处理指定 PR
$pr-fix --human-only             # 只处理 jhfnetboy 自己的 PR
$pr-fix --bot-only               # 只处理 dependabot/renovate PR
```
...
│   ├── poll_fix_queue.py       ← $pr-fix 入口：jhfnetboy RC/comment + bot PRs 待 review（--human-only/--bot-only）
