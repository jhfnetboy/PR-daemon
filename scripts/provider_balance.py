#!/usr/bin/env python3
"""
PR-Daemon balance bridge — query usage/balance for all three providers.
...
Usage:
  python3 balance.py                       # one-shot, human-readable
  python3 balance.py --watch               # refresh every 30 min (default)
  python3 balance.py --watch 10            # refresh every 10 min
  python3 balance.py --compact             # one-line per provider
  python3 balance.py --json                # raw JSON

No secrets ever leave this machine.
"""

import json, os, ssl, subprocess, sys, time, glob
...
def http_get(url, headers=None):
    """Return (status, parsed_json). Never raises on HTTP errors."""
...
def window_pct_bar(w):
    """Render a usage window as:  45.2%  █████████░░░░░░░░░░░  resets in 3h15m"""
    if not w: return "—"
...
def fetch_deepseek():
    key = load_env_key()
    if not key: return {"error": "no DEEPSEEK_API_KEY"}
...
def fetch_codex():
    ...
    try:
        ...
        token = None
    if not token: return {"error": "no codex auth"}
    ...
    return {
        ...
        "five_hour":    w("primary_window"),
        "weekly":       w("secondary_window"),
    }
...
def render_all(ds, cx, cl, prev_snap):
    """Human-readable multi-provider block."""
    ...
    if "error" in ds:
        ...
        available = "✓ online" if ds.get("available") else "✗ offline"
        lines.append(f"     status      {available}")
        lines.append(f"     CNY left    {ds['cny_left']}   (topped_up: {ds['cny_topped']}  granted: {ds['cny_granted']})")
        if prev_snap and prev_snap.get("deepseek") and "error" not in prev_snap["deepseek"]:
            prev_total = float(prev_snap["deepseek"].get("cny_left", 0))
            curr_total = float(ds.get("cny_left", 0))
...
def collect():
    """Fetch all three providers. Returns dict."""
    ds = fetch_deepseek()
    cx = fetch_codex()
    cl = fetch_claude()
    return {"ts": int(time.time()), "deepseek": ds, "codex": cx, "claude": cl}
    ...
    if watch_mode:
        ...
        try:
            while True:
                ...
                # reload prev each cycle for fresh delta
                state["prev_snap"] = load_state()
                do_query()
